...
;(function($, window, document) {
  ...
  var FJS = function(records, container, options) {
    ...
    var init_page = this.opts.init_page;
  ...
  F.initPagination = function(init_page){
    var self = this,
  ...
  var Paginator = function(recordsCount, opts, onPagination, init_page) {
    var paginationView;
    ...
    this.render();
    this.bindEvents();
    this.buildPageDropdown();
  };
  
  var P = Paginator.prototype;

  // Build 'page dropdown' by A. Gantikow
  P.buildPageDropdown = function(){
    var total_pages = this.totalPages();
    $('.page-ul').empty();
    for(var i = 1; i <= total_pages; i++) {      
      $('.page-ul').append('<li id="page_'+i+'">'+i+'</li>');
    }   
  };

  P.updatePageDropdown = function(page){
    this.buildPageDropdown();
    var total_pages = this.totalPages();
  ...
  P.bindEvents = function(){
    ...
    $(this.opts.container).on('click', '[data-page]', function(e){
      ...
      e.preventDefault();
      // if last page reached, scroll to results
      if (self.currentPage == self.totalPages()) {
        scrollToHash($('#articles').last());
      }      
    });

    // Pagination dropdown by A. Gantikow
    $('.page-ul').on('click', 'li', function(e){
      self.setCurrentPage($(this).text());
      e.preventDefault();
    });
  ...
  P.setCurrentPage = function(page){
    ...
    this.paginate(page);
    // Page query management by A. Gantikow
    //if(page != 1 || uri.search(true).page >= 1) {
    uri.removeQuery("page");
    uri.addQuery("page", page);
    set_history(uri.query());
    this.updatePageDropdown(page);
  };
  ...
  P.setPerPage = function(value){
    ...
    this.setCurrentPage(this.currentPage);
    this.updatePageDropdown(this.currentPage);
  }
  ...
  views['per_page'] = '<select size="1" name="per_page" data-perpage="true" class="per-page btn btn-default dropdown-toggle">  <% for(var i = 0; i < values.length; i++ ){ %>    <option value="<%= values[i] %>"><%= values[i] %></option>  <% } %></select>'; 
